...
randomTrainingSamples = np.random.choice(n2, 50, replace=False)
x_train = Xtr[randomTrainingSamples]
y_train = Ytr[randomTrainingSamples]

# subsets for validation
randomTestingSamples = np.setdiff1d(np.arange(n2), randomTrainingSamples)
x_validation = Xtr[randomTestingSamples]
y_validation = Ytr[randomTestingSamples]
#############placeholder end #######################
...
for kernel_value in kernel_types:
    # your own codes
    svm_model = svm.SVC(kernel=kernel_value, C=1)
    svm_model.fit(X=x_train, y=np.ravel(y_train))
    error = 1. - svm_model.score(x_validation, y_validation)
...
best_kernel = 'linear'
best_c = 4 # poly had many that were the "best"
...
correctClassified_samples = X_test[Y_test == y_pred]
correctClassified_samples[0:5,:]
# Failure samples: samples for which you model can not correctly predict their labels
misclassified_samples = X_test[Y_test != y_pred]
misclassified_samples[0:5,:]
#############placeholder end #######################
